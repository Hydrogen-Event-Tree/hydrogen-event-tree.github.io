<p align="center">
  <img src="media/screenshot1.png" width="48.4%"/>
  <img src="media/screenshot2.png" width="50.6%"/>
</p>
...
The dashboard loads `src/events/events-manifest.json`. The manifest lists one or more model entries:
  ...
  "models": [
    {
      "id": "qwen3-14b",
      "name": "Qwen3-14B",
      "events_path": "src/events/events-qwen3-14b.json",
      "model": "qwen3:14b"
    }
  ],
...
Each `events_path` points to a JSON array produced by `src/main.py`. To generate (or refresh) these files, set `gen = 1` in `src/main.py`. It will run `process_events()` for every entry in `LLMS`, write files to `src/events/events-<id>.json`, and rebuild `src/events/events-manifest.json`. If you already have the JSON files, leave `gen = 0` and the script will just start the webserver on `localhost:4000`.

### Models

Models are configured in `LLMS` at the top of `src/main.py`. For local inference, `ollama` must be installed both as a pip package and as a running service (https://ollama.com/). For OpenRouter-backed entries, set `OPENROUTER_API_KEY` in `src/main.py` before running the generator, and ensure the `openai` package is installed.
